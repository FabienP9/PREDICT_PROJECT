...
def transform_games_to_list(df_games: pd.DataFrame) -> str:
...
@config_decorators.exit_program(log_filter=lambda args: {k: args[k] for k in ('sr_gameday_output_init',) })
def get_parameters(sr_snowflake_account: pd.Series, sr_gameday_output_init: pd.Series) -> dict:
    ...
    df_games_opening = snowflake_execute(sr_snowflake_account,sql.VW_GAME_QUERY,sql.DATABASE,(sr_gameday_output_init['SEASON_ID'],sr_gameday_output_init['GAMEDAY']))
    param_dict['NB_GAMES_OPENING'] = len(df_games_opening)
    if param_dict['NB_GAMES_OPENING'] > 0:
        param_dict['CALENDAR_GAMES_OPENING'], FIRSTGAMETIME_DICT_OPENING = transform_games_to_calendar(df_games_opening)
        ...
        param_dict['LIST_GAMES_OPENING'] = transform_games_to_list(df_games_opening)
    
    #parameters from gamedays already opened (at defined date)
    defined_date = datetime.now(timezone.utc).strftime('%Y-%m-%d')
    df_games_opened = snowflake_execute(sr_snowflake_account,sql.VW_GAME_OPENED_QUERY,sql.DATABASE,(sr_gameday_output_init['SEASON_ID'],
...
@config_decorators.exit_program(log_filter=lambda args: {k: args[k] for k in ('country','forum','sr_gameday_output_init') })
def create_message(param_dict: dict, template: str, translations_dict: dict, country: str, forum:str, sr_gameday_output_init: pd.Series) -> Tuple[str,str,str]:
    ...
    content = content.replace("#MESSAGE_PREFIX_PROGRAM_STRING#",var.MESSAGE_PREFIX_PROGRAM_STRING)
    ...
    # ... and on condition
    content = output.replace_conditionally_message(content, 
    ...
    file_name = output.define_filename("forumoutput_inited", sr_gameday_output_init, 'txt', country, forum)
...
def process_output_message_inited(context_dict: dict, sr_gameday_output_init: pd.Series):
    ...
    logging.info("OUTPUT -> GENERATING INIT MESSAGE [START]")
    ...
    df_topics = snowflake_execute(sr_snowflake_account,sql.VW_TOPIC_INIT_QUERY,sql.DATABASE,(sr_gameday_output_init['SEASON_ID'],))
    list_countries_forums = (df_topics[['FORUM_COUNTRY', 'FORUM_SOURCE']].drop_duplicates().values.tolist())

    # we get all parameters needed
    param_dict = get_parameters(sr_snowflake_account,sr_gameday_output_init)
    logging.info("OUTPUT -> PARAM RETRIEVED")

    # we create messages from parameters for each country
    message_args= [(param_dict,context_dict['str_output_gameday_init_template_'+country.lower()],
                    context_dict['lst_output_gameday_template_translations'],country,forum,
                    sr_gameday_output_init) for (country,forum) in list_countries_forums]
    ...
    logging.info("OUTPUT -> GENERATING INITED MESSAGE [DONE]")
