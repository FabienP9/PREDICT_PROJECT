...
def get_calendar(sr_snowflake_account: pd.Series) -> pd.DataFrame:
        ...
        df_calendar = snowflake_execute(sr_snowflake_account,sql.CALENDAR_QUERY, sql.DATABASE)

        logging.info("SNOWFLAKE -> GETTING RUN CALENDAR [DONE]")
        return df_calendar

@config_decorators.exit_program(log_filter=lambda args: {'columns_df_calendar': args['df_calendar'].columns.tolist(), 'columns_df_task_done': args['df_task_done'].columns.tolist() })
...
def update_nextrun(df_calendar: pd.DataFrame, df_task_done: pd.DataFrame) -> str:
    ...
    files_manipulation.create_txt(os.path.join(var.TMPF,'next_run_time_utc.txt'),str_next_run_utc)
...
def add_task_to_taskdone(sr_output_need: pd.Series, df_task_done: pd.DataFrame) -> pd.DataFrame:
    ...
    files_manipulation.create_csv(os.path.join(var.TMPF,"task_done.csv"),df_task_done_modified,is_to_encapsulate=var.TASK_DONE_ENCAPSULATED)
...
def update_calendar_related_files(called_by: str, sr_snowflake_account: pd.Series, df_task_done: pd.DataFrame, sr_output_need: pd.Series | None = None):
    ...
    """
        ...
        Args:
            called_by (string): The entry point function calling this one, task_done will be updated only with main
            sr_snowflake_account (series - one row) : Contains the snowflake account parameters to run a query
            df_task_done (dataframe) : Contains task already done
