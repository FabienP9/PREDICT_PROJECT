...
def test_translate_string():
    ...
    result = output_message_actions.translate_string(content, country, forum, translations)
...
def test_translate_df_headers_one_level():
    ...
    result = output_message_actions.translate_df_headers(df, country, forum, translations)
...
def test_display_rank():
    ...
    result_df = output_message_actions.display_rank(df, 'RANK')
...
def test_calculate_and_display_rank():
    ...
    columns = ['score']
    result = output_message_actions.calculate_and_display_rank(df, columns)
...
def test_capture_df_oneheader():
    ...
    with patch.object(output_message_actions.fileA,"create_jpg"): 
        output_message_actions.capture_df_oneheader(df, capture_name)

def test_capture_scores_detailed():

    # this test the function capture_scores_detailed
    df = pd.read_csv("materials/output_message_calculated_scores_details.csv", header=[0, 1])
    capture_name = "mycapture"

    with patch.object(output_message_actions.fileA, 'create_jpg'):
        output_message_actions.capture_scores_detailed(df, capture_name)
...
def test_manage_df():
    ...
    with patch.object(output_message_actions,"translate_df_headers", return_value=translated_df) as mock_translate, \
         patch.object(output_message_actions,"define_filename", return_value=full_capture_name) as mock_define_filename, \
         patch.object(output_message_actions,"capture_df_oneheader") as mock_capture_oneheader, \
         patch.object(output_message_actions,"capture_scores_detailed") as mock_capture_detailed, \
         patch.object(output_message_actions,"push_capture_online", return_value=expected_url) as mock_push, \
         patch.object(output_message_actions,"config") as mock_config:

        mock_config.TMPF = "/tmp"

        result = output_message_actions.manage_df(
            df=df,
            country=country,
            forum=forum,
...
def test_generate_output_message_init():
    ...
    with patch.object(output_message_actions,"snowflake_execute", return_value= mock_df_gameday_output), \
         patch.object(output_message_actions.outputAI,"process_output_message_inited") as mock_inited, \
         patch.object(output_message_actions.outputAC,"process_output_message_calculated") as mock_calc:
        
        output_message_actions.generate_output_message(context_dict)
