...
{% macro calendar_read_gameday_beginning(datepart, value_shift,hourtime_utc)%}
...
{% macro calendar_read_gameday_ending(datepart, value_shift,hourtime_utc)%}
    ...
    */
    SELECT 
        gameday_read.GAMEDAY_KEY,
        TO_TIMESTAMP(TO_DATE(DATEADD({{datepart}}, {{value_shift}}, gameday_read.END_DATE_UTC)) || ' ' || '{{hourtime_utc}}') AS TS_TASK_UTC
    FROM
        {{ref('consumpted_gameday')}} gameday_read
    WHERE
        gameday_read.BEGIN_TIME_UTC IS NOT NULL
