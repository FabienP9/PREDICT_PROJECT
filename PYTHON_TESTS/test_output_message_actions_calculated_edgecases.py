...
def test_get_calculated_games_result_negative_result():
    ...
    sr_snowflake_account = pd.read_csv("materials/snowflake_account_connect.csv").iloc[0]
    sr_gameday_output_calculate = pd.read_csv("materials/sr_gameday_output_calculate.csv").iloc[0]
    s_expected = read_txt("materials/edgecases/output_message_calculated_games_result_negative.txt")
...
def test_get_calculated_scores_detailed_missing_split():
    ...
    with patch.object(output_message_actions_calculated, "snowflake_execute", return_value=mock_df_predict_games):
        assertExit(lambda: output_message_actions_calculated.get_calculated_scores_detailed(sr_snowflake_account, sr_gameday_output_calculate))
...
def test_get_calculated_scores_global_missing_cols():
    ...
    with patch.object(output_message_actions_calculated.outputA, "display_rank", return_value=mock_df_rank):
        assertExit(lambda: output_message_actions_calculated.get_calculated_scores_global(df_userscores_global))
...
def test_get_calculated_scores_gameday_missing_column():
    ...
    with patch.object(output_message_actions_calculated, "snowflake_execute", return_value=mock_df_userscores_gameday), \
        patch.object(output_message_actions_calculated.outputA, "display_rank", return_value=mock_df_userscores_gameday_ranked):
            assertExit(lambda: output_message_actions_calculated.get_calculated_scores_gameday(sr_snowflake_account, sr_gameday_output_calculate))
...
def test_get_calculated_scores_average_missing_column():
    ...
    with patch.object(output_message_actions_calculated.outputA, 'calculate_and_display_rank', return_value= mock_df_rank):
        assertExit(lambda: output_message_actions_calculated.get_calculated_scores_average(nb_prediction, df_userscores_global))
...
def test_get_calculated_list_gameday_missing_columns():
    ...
    assertExit(lambda: output_message_actions_calculated.get_calculated_list_gameday(df_list_gameday))
...
def test_get_calculated_list_gameday_empty_dataframe():
    ...
    result = output_message_actions_calculated.get_calculated_list_gameday(df_list_gameday)
    assert result == ""
...
def test_get_mvp_month_race_figure_invalid_points():
    ...
    expected_str = read_txt("materials/edgecases/output_message_calculated_mvp_race_figures_invalid_points.txt")
...
def test_get_mvp_compet_race_figure_missing_key():
    ...
    with patch.object(output_message_actions_calculated, "snowflake_execute", return_value=mock_df_compet_mvp):
        assertExit(lambda: output_message_actions_calculated.get_mvp_compet_race_figure(sr_snowflake_account, sr_gameday_output_calculate))
...
def test_list_mvp_compet_race_gameday_missing_columns():
    ...
    with patch.object(output_message_actions_calculated, "snowflake_execute", return_value=mock_df_list_gameday):
        assertExit(lambda: output_message_actions_calculated.list_mvp_compet_race_gameday(sr_snowflake_account,sr_gameday_output_calculate))
...
def test_get_calculated_parameters_no_predictchamp():
    ...
    with patch.object(output_message_actions_calculated,"get_calculated_games_result", return_value=(mock_str_games_result, 2)), \
         ...
         patch.object(output_message_actions_calculated,"get_calculated_predictchamp_result") as mock_predictchamp_result, \
         patch.object(output_message_actions_calculated,"get_calculated_predictchamp_ranking") as mock_predictchamp_ranking, \
...
def test_get_calculated_parameters_missing_key():
    ...
    with patch.object(output_message_actions_calculated,"get_calculated_games_result", return_value=(mock_str_games_result, 2)), \
         ...
         patch.object(output_message_actions_calculated,"get_calculated_scores_gameday", return_value=(mock_df_scores_gameday, 2)), \
         patch.object(output_message_actions_calculated,"get_calculated_list_gameday", return_value=mock_str_list_gameday), \
         patch.object(output_message_actions_calculated,"get_calculated_predictchamp_result"), \
         patch.object(output_message_actions_calculated,"get_calculated_predictchamp_ranking"), \
         patch.object(output_message_actions_calculated,"get_calculated_correction", return_value=(mock_str_correction, 2)), \
         patch.object(output_message_actions_calculated,"get_mvp_month_race_figure", return_value=("__L__MONTH_01__L__", mock_str_mvp_month, 2)), \
         patch.object(output_message_actions_calculated,"list_mvp_month_race_gameday", return_value=(mock_str_mvp_month_gameday)), \
         patch.object(output_message_actions_calculated,"get_mvp_compet_race_figure", return_value=("__L__REGULAR SEASON__L__", mock_str_mvp_compet, 2)), \
        ...
        assertExit(lambda: output_message_actions_calculated.get_calculated_parameters(sr_snowflake_account, sr_gameday_output_calculate))
...
def test_process_output_message_calculated_posting_fails():
    ...
    with patch.object(output_message_actions_calculated,"snowflake_execute", return_value=mock_df_topics), \
         patch.object(output_message_actions_calculated,"get_calculated_parameters", return_value=mock_params_retrieved), \
         patch.object(output_message_actions_calculated.config,"multithreading_run", side_effect=Exception("boom")):

        assertExit(lambda: output_message_actions_calculated.process_output_message_calculated(context_dict, sr_gameday_output_calculate))
