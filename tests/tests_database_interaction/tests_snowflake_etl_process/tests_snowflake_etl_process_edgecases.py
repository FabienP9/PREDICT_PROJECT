...
def test_delete_table_data_executes_expected_queries(read_csv):
    ...
    with patch.object(snowflake_etl_process,'snowflake_execute') as mock_exec:
        ...
        calls = [call(sr_snowflake_account, ANY,snowflake_etl_process.sql.DATABASE), 
                 call(sr_snowflake_account, ANY,snowflake_etl_process.sql.DATABASE)]
        mock_exec.assert_has_calls(calls, any_order=False)

def test_update_snowflake_from_python_encapsulated(read_csv):
...
def test_update_snowflake_from_dbt_failure(assert_exit,read_csv):
    ...
    with patch.object(snowflake_etl_process.subprocess,'run') as mock_run, \
        ...
        assert_exit(lambda: snowflake_etl_process.update_snowflake_from_dbt(called_by,sr_snowflake_account,df_paths,lst_dbt_tables))

def test_update_snowflake_initsnowflake_runs_python_and_dbt(read_csv):
...
def test_update_snowflake_main_with_empty_lists(read_csv):
