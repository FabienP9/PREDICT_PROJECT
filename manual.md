...
- [Input Variables](#inputparameters)
- [DropBox repository tree](#dropboxtree)
- [Files required in DropBox](#filesrequired)
- [How to add sources and competition to scope](#addtoscope)
...
- **season**: A season is a specific sport league during a seasonal year from a specific country (ex: The Elite league of basketball in France in 2024-2025)
- **competition**: A competition is a part of a season where rules are consistent (ex: Regular Season, or Play-Offs)
- **gameday**: A gameday is a set of games from a competition grouped together, where each team can play maximum once (ex: Gameday 3),usually designed by the sport league organization. Usually, all games of a gameday occur within the same time frame.
...
- Games in the prediction championship
    ...
    For other competitions, depending on the ranking of the prediction championship, games might be different. For exemple for playoffs, the qualified TOP8 will almost certainly be different than the one from the predicted league.
...
For the last gameday of each month —or the last gameday of a competition— the program posts the results along with the total points earned during that period or competition for each player (both individual and team chosen-based wins/losses).  
An election for the MVP takes place based on these figures.

## Input Variables<a name="inputparameters"></a>

The software uses a set of environment variables as input, provided as GitHub secrets (see readme.md file for more details).  

Plus,considering the context of run:
...
- <a name="overwritegamesstatus"></a>**OVERWRITE_GAMES_STATUS** (0/1): When calculating results, if 1, the program won't check if all games are over, before updating SnowFlake database with predictions calculations. Else it will raise an error and exit the program.
...
This tree can totally be changed by modifying both [paths file](#pathsfile) and Python global variables file in *src.predict_core.config.config_variables.config_global_variables.py*
...
│       # Migrated at the beginning of run for both Prod or Test runs into Prod/current/inputs/manual & Test/current/inputs/manual for possible download downstream 
├── Prod/ # Folder for production environment files
│   ├── local_manual_inputs/
│   │   ├── # Manual inputs (see Files Required section) 
│   │       # related to the production environment run only
│   │       # Migrated at the beginning of the run into Prod/current/inputs/manual for possible download downstream
│   ├── current/ # Files related to the most recent software run, or the running instance
│   │   ├── inputs/ 
│   │   │   ├── manual/ 
│   │   │   │   ├── # Manual input files used for the last run, or currently running instance
│   │   │   ├── calculated/ 
│   │   │   │   ├── # Files calculated at the beginning of the run 
│   │   │   │       # Used downstream during execution 
│   │   │   │       # Some of them must be created with their headers initially (see Files Required section)
│   │   ├── outputs/
│   │   │   ├── database/ 
...
Some files must be written manually as input of the program, others at least created and initialized with their headers for the software to run. 
Example of each file is copied under *file_exemples/* folder

### Documents files required, in DropBox *docs/* folder:

- <a name="pathsfile"></a>**paths.csv**. Defines where each file is located on DropBox, in which condition it needs to be downloaded by the program, and which filter to apply on their content, before using them as Snowflake inputs. Values can be change by software administrator if changing Python parameters in *src.predict_core.config.config_variables.config_global_variables.py** and changing [DropBox tree](#dropboxtree).
    ...
    - **FILTERING_CATEGORY**: Defines when filtering applies (categories defined in *src.predict_core.config.config_variables.config_global_variables.py**).  
    ...
    - **DOWNLOAD_CATEGORY**: Defines when the file must be downloaded (see *src.predict_core.config.config_variables.config_global_variables.py**).  
...
## How to add sources and competition to the scope<a name="addtoscope"></a>
...
- To prepare a new competition
    ...
    - develop a Python module *src.predict_core.games_details_extraction.games_details_extraction_xxx.games_details_extraction_xxx.py* with *xxx* being the source name of the competition.
    - import the created module in the module *src.predict_core.games_details_extraction.games_details_extraction.py* and add the module main function to Python dictionary *game_info_functions* for it to be called
    - add *xxx_URL* and possibly credentials (*xxx_USERNAME* and *xxx_PASSWORD*) in  
        → GitHub secrets  
        → GitHub Actions YAML files (*.github/workflows*) for the program to get them automatically  
        → in *src.predict_core.config.config_variables.config_environment_variables.py/check_environment_variable* function for the program to assess their initialization at the beginning of run    
        → in *tests.tests_config.tests_config_variables.tests_config_environment_variables(_edegecases).py/tests_check_environment_variable* function to validate the good run of the function *check_environment_variable*  
...
- To link the competition with new topics
    ...
    - develop Python modules:    
        →  *src.predict_core.forums_interaction.forums_interaction_xxx.messages_details_extraction_xxx.py* for message extraction  
        →  *src.predict_core.forums_interaction.forums_interaction_xxx.messages_posting_process_xxx.py* for message posting  
    with *xxx* being the source name of the forum.  
    - import the created module in modules:    
        →  *src.predict_core.forums_interaction.messages_details_extraction.py*  
        →  *src.predict_core.forums_interaction.messages_posting_process.py*  
         and add module main functions to Python dictionary *message_info_functions* and *post_info_functions* for it to be called  
    - add *xxx_URL* and possibly credentials (*xxx_USERNAME* and *xxx_PASSWORD*) in  
        → GitHub secrets  
        → GitHub Actions YAML files (*.github/workflows*) for the program to get them automatically  
        → in *src.predict_core.config.config_variables.config_environment_variables.py/check_environment_variable* function for the program to assess their initialization at the beginning of run    
...
- Eventually, to add the new competition and topics to the Snowflake database, the software administrator must:  
    - run [the "competition integration" entry point](#competitionintegration)  
...
During the [main run](#mainrun) or ["competition integration" run](#competitionintegration), DBT performs DML operations on Snowflake database:  
...
- TASK_RUN = 'CALCULATE': planned two hours after the beginning of last game of the gameday, to perform and post calculations of predictions. If the last game is not finished yet, the extraction will return an error, and retry for the next automatic run, unless [OVERWRITE_GAMES_STATUS](#overwritegamesstatus) = 1
...
- <a name="initsnowflake"></a>Snowflake account initialization: Creates two new databases (production and test) on a snowflake account, populating tables with csv files from Dropbox folder *current/outputs/database/*
    - Can be run locally: 
        ```
            python -m src.predict_core.entry_point.snowflake_account_initialization
        ```
    - Can be run through GitHub actions through the workflow *gitrun_snowflake_account_initialization.yml*

- <a name="initcompet"></a>Competition integration: [Add a new competition](#addtoscope) to the scope of the program (production database and test database) with its own games and calculting its [planned calendar](#calendar). The file [next_run_time_utc.txt](nextruntimeutc) is updated based on this new calendar added.  
The input file [competition.csv](#competition) must have a row with this new competition with IS_TO_LOAD = 1  
    - Can be run locally: 
        ```
            python -m src.predict_core.entry_point.competition_integration
        ```
    - Can be run through GitHub actions through the workflow *gitrun_competition_integration.yml*

- <a name="generateplayoffs"></a>Generate a playoffs table results for playoffs competition: Creates a jpg graphical picture for prediction championship results, according to values in [the input file playoffs_table.txt](#playoffstable).  
    - Can be run locally:  
        ```
            python -m src.predict_core.entry_point.playoffs_table_generation

        ```
    - Can be run through GitHub actions through the workflow *gitrun_playoffs_table_generation.py*
...
After running [an entry point](#usage) through GitHub actions, GitHub will send an email to the recipient to inform them of the success or failure.  
