...
to generate the bracket for the prediction championship playoff
'''
...
from ..files_manipulation.external_files_interaction import imgbb_captures_interaction as imgbb
...
def display_textbox(ax: Axes, column: int, line: int, str_matchup: str, str_result: str,size: int = 13) -> Axes:
    ...
    if str_result == "":
        ax.text(column+0.2,line,str_matchup, fontsize=size, fontweight="bold", ha="left",bbox=dict(facecolor='white', edgecolor='black'),zorder=10) # NOSONAR
    else:
        team_box = TextArea(str_matchup, textprops=dict(fontsize=size, fontweight="bold")) # NOSONAR
        result_box = TextArea(str_result, textprops=dict(fontsize=size, fontweight="bold", fontstyle="italic")) # NOSONAR
        ...
        ab = AnnotationBbox(
            ...
            box_alignment=(0, 0.5),
            frameon=True,
            bboxprops=dict(facecolor='white', edgecolor='black', boxstyle='round,pad=0.3'), # NOSONAR
            zorder=10)
        ax.add_artist(ab)
...
def draw_playoffs_image():
    ...
    context_dict.update(dropbox.download_file(dropbox_file_path = var.PLAYOFFS_TABLE_CODE,
    ...
    if (playoffs_matchups[len(playoffs_matchups)-1][0]) != s:
        ...
        dropbox.download_file(dropbox_file_path = var.TROPHY_FILE_PATH,
                                        local_folder = var.TMPF,
                                        is_encapsulated=0,
                                        is_path_abs=1)
        
        
        trophy_img = Image.open(var.TMPF+"/Trophy.JPG").convert('RGBA')
    ...
    for round, value in enumerate(playoffs_round):
        ax.text(1+3*round,11, value, fontsize=16, fontweight="bold", ha="left")
    ...
    file_path = os.path.join(var.TMPF,"playoffs_table_"+datetime.now(timezone.utc).strftime("%Y_%m_%d_%H_%M_%S")+".jpg")
    files_manipulation.create_jpg(file_path,fig)
    
    # we push it online on ImgBB
    image_url = imgbb.push_capture_online(file_path)
    logging.info(image_url)
    create_json_file_email(image_url = image_url)
