...
def test_onError_final_execute_parametrizes_file():
    ...
    with patch.object(config,"dbt_profiles_path", mock_profile_file):
...
def test_check_environment_variable():
    ...
    # this test functions create_local_folder and check_environment_variable
    env_values = {
        "IS_TESTRUN": "1",
        "IS_OUTPUT_AUTO": "1",
    ...
    with patch.dict(os.environ, env_values, clear=False):
        config.check_environment_variable("main")
...
if __name__ == '__main__':
    ...
    test_suite.addTest(unittest.FunctionTestCase(test_check_environment_variable))
