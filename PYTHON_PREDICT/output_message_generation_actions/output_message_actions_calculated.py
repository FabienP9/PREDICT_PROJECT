...
def get_mvp_compet_race_figure(sr_snowflake_account: pd.Series, sr_gameday_output_calculate: pd.Series) -> Tuple[str, int]:
    ...
    df_compet_mvp = snowflake_execute(sr_snowflake_account,sqlQ.qMVPRace_Compet_figures,(sr_gameday_output_calculate['SEASON_ID'],sr_gameday_output_calculate['COMPETITION_LABEL']))
...
def get_calculated_parameters_df_management(param_dict: dict, sr_gameday_output_calculate: pd.Series, country: str, forum: str, translations_dict: dict) -> dict:
    ...
    param_df_dict = {}
    param_df_dict['SCORES_DETAILED_DF_URL_'+country+'_'+forum] = outputA.manage_df(param_dict['SCORES_DETAILED_DF'], country, forum, "table_score_details", sr_gameday_output_calculate, translations_dict)
    param_df_dict['SCORES_GLOBAL_DF_URL_'+country+'_'+forum] = outputA.manage_df(param_dict['SCORES_GLOBAL_DF'], country, forum, "table_global_scores", sr_gameday_output_calculate, translations_dict)
    param_df_dict['SCORES_GAMEDAY_DF_URL_'+country+'_'+forum] = outputA.manage_df(param_dict['SCORES_GAMEDAY_DF'], country, forum, "table_gameday_scores", sr_gameday_output_calculate, translations_dict)
    
    if param_dict['IS_FOR_RANK'] == 0:
        param_df_dict['RANK_PREDICTCHAMP_DF_URL_'+country+'_'+forum] = None
    else:
        param_df_dict['RANK_PREDICTCHAMP_DF_URL_'+country+'_'+forum] = outputA.manage_df(param_dict['RANK_PREDICTCHAMP_DF'], country, forum, "table_predictchamp_ranking", sr_gameday_output_calculate, translations_dict)
...
def create_calculated_message(param_dict: dict, template:str, translations_dict: dict, country: str, forum: str, sr_gameday_output_calculate: pd.Series) -> Tuple[str,str]:
...
def process_output_message_calculated(context_dict: dict, sr_gameday_output_calculate: pd.Series):
    ...
    message_args= [(param_dict,context_dict['str_output_gameday_calculation_template_'+country],context_dict['lst_output_gameday_template_translations'],country,forum,sr_gameday_output_calculate) for (country,forum) in list_countries_forums]
