...
def test_snowflake_connect(read_csv):
    ...
    with patch.object(snowflake_connection_execution.snowflake.connector,'connect', return_value=mock_conn) as mock_connect, \
        ...
        conn = snowflake_connection_execution.snowflake_connect(sr_snowflake_account)

        assert conn == mock_conn
        mock_connect.assert_called_once()

def test_snowflake_execute(read_csv):
...
def test_snowflake_execute_script_uses_prod_db(read_csv):
...
def test_snowflake_execute_script_uses_test_db(read_csv):
