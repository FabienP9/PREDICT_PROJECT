'''
    ...
    This module:
    - choose which message to post (gameday prediction results calculated or predictions template inited)
...
from .....files_manipulation.external_files_interaction.imgbb_captures_interaction import push_capture_online
...
from . import output_message_inited_generation as output_i
from . import output_message_calculated_generation as output_c

logging.basicConfig(level=logging.INFO)

@config_decorators.exit_program(log_filter=lambda args: {k: args[k] for k in ('country','forum')})
def translate_string(content: str, country: str, forum: str, translations: dict) -> str:
...
@config_decorators.exit_program(log_filter=lambda args: {k: args[k] for k in ('country','forum')})
def translate_df_headers(df: pd.DataFrame, country: str, forum: str, translations: dict) -> pd.DataFrame:
...
@config_decorators.exit_program(log_filter=lambda args: {k: args[k] for k in ('begin_tag','end_tag','condition')})
...
def define_filename(input_type: str, sr_gameday_output: pd.Series, extension: str, country: str | None = None, forum: str | None = None) -> str:
    ...
    '''
        ...
        Inputs:
            ...
            sr_gameday_output (series - one row) containing elements for the name
    ...
    file_name = input_type + '_' + sr_gameday_output['SEASON_ID'] + '_' + sr_gameday_output['GAMEDAY'] 
...
@config_decorators.exit_program(log_filter=lambda args: {'columns_df': args['df'].columns.tolist(), 'columns': args['columns'] })
...
@config_decorators.exit_program(log_filter=lambda args: {'columns_df': args['df'].columns.tolist(), 'capture_name': args['capture_name'] })
def capture_df_oneheader(df: pd.DataFrame, capture_name: str):
    ...
    files_manipulation.create_jpg(os.path.join(var.TMPF,capture_name),fig)

@config_decorators.exit_program(log_filter=lambda args: {'columns_df': args['df'].columns.tolist(), 'capture_name': args['capture_name'] })
def capture_scores_detailed(df: pd.DataFrame, capture_name: str): # NOSONAR
    ...
    for row_idx, row in enumerate(table_data):
        for col_idx, cell in enumerate(row):
            if row_idx == 0 and col_idx > 0:  # Handle merged effect for the first-level header
                if col_idx == 1 or cell != row[col_idx - 1]:
...
@config_decorators.exit_program(log_filter=lambda args: {k: args[k] for k in ('country','forum','capture_name', 'sr_gameday_output')})
def manage_df(df: pd.DataFrame, country: str, forum: str, capture_name: str, sr_gameday_output: pd.Series,translations_dict: dict) -> str:
    ...
    '''
        ...
        Raises:
            Exits the program if error running the function (using decorator)
    '''
    df = translate_df_headers(df, country, forum, translations_dict)
    ...
    local_path = os.path.join(var.TMPF, full_capture_name)
...
def generate_output_message(context_dict: dict):
    ...
    df_gameday = snowflake_execute(context_dict['sr_snowflake_account_connect'],sql.VW_GAMEDAY_QUERY,sql.DATABASE,(sr_output_need['SEASON_ID'],sr_output_need['GAMEDAY']))
    
    has_to_post_inited_message = False
    has_to_post_calculated_message = False
    
    #We choose what needs to be posted based on TASK_RUN
    if len(df_gameday) > 0 and sr_output_need['TASK_RUN']  == var.TASK_RUN_MAP["INIT"]:
        has_to_post_inited_message = True
    
    if len(df_gameday) > 0 and sr_output_need['TASK_RUN']  == var.TASK_RUN_MAP["CALCULATE"]:
    ...
    if has_to_post_inited_message:
        return output_i.process_output_message_inited(context_dict,df_gameday.iloc[0])

    if has_to_post_calculated_message:
       return output_c.process_output_message_calculated(context_dict,df_gameday.iloc[0])
