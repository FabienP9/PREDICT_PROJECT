...
from . import output_message_generation as output
from . import output_message_generation_sql_queries as sql
...
def get_scores_detailed(sr_snowflake_account: pd.Series, sr_gameday_output_calculate: pd.Series) -> Tuple[pd.DataFrame,int]:
    ...
    df_predict_games = snowflake_execute(sr_snowflake_account,sql.VW_PREDICT_GAME_QUERY,sql.DATABASE,(sr_gameday_output_calculate['SEASON_ID'],sr_gameday_output_calculate['GAMEDAY']))
...
@config_decorators.exit_program(log_filter=lambda args: {'columns_df_userscores_global': args['df_userscores_global'].columns.tolist(),})
def get_scores_global(df_userscores_global: pd.DataFrame) -> Tuple[pd.DataFrame,int]:
...
@config_decorators.exit_program(log_filter=lambda args: {k: args[k] for k in ('sr_gameday_output_calculate',)})
def get_scores_gameday(sr_snowflake_account: pd.Series, sr_gameday_output_calculate: pd.Series) -> Tuple[pd.DataFrame,int]:
    ...
    df_userscores_gameday = snowflake_execute(sr_snowflake_account,sql.VW_USER_SCORES_GAMEDAY_QUERY,sql.DATABASE,(sr_gameday_output_calculate['SEASON_ID'],sr_gameday_output_calculate['GAMEDAY']))
    df_userscores_gameday = output.display_rank(df_userscores_gameday,'RANK')
...
@config_decorators.exit_program(log_filter=lambda args: {'nb_prediction': args['nb_prediction'], 'columns_df_userscores_global': args['df_userscores_global'].columns.tolist() })
def get_scores_average(nb_prediction: int, df_userscores_global: pd.DataFrame) -> Tuple[str,int, int]:
...
@config_decorators.exit_program(log_filter=lambda args: {'columns_df_gamepredictchamp': args['df_gamepredictchamp'].columns.tolist(), 'sr_gameday_output_calculate': args['sr_gameday_output_calculate'] })
def get_predictchamp_result(df_gamepredictchamp: pd.DataFrame, sr_snowflake_account: pd.Series, sr_gameday_output_calculate: pd.Series) -> str:
    ...
    df_gamepredictchamp_detail = snowflake_execute(sr_snowflake_account,sql.VW_GAME_PREDICTCHAMP_DETAILS_QUERY,sql.DATABASE,(sr_gameday_output_calculate['SEASON_ID'],sr_gameday_output_calculate['GAMEDAY']))
...
@config_decorators.exit_program(log_filter=lambda args: {k: args[k] for k in ('sr_gameday_output_calculate',)})
def get_predictchamp_ranking(sr_snowflake_account: pd.Series, sr_gameday_output_calculate: pd.Series) -> pd.DataFrame:
    ...
    df_teamscores = snowflake_execute(sr_snowflake_account,sql.VW_TEAM_SCORES_QUERY,sql.DATABASE,(sr_gameday_output_calculate['SEASON_ID'],))

    #we add a rank per team - first by percentage of win, then by points difference
    df_teamscores = output.display_rank(df_teamscores,'RANK')
    
    df_teamscores = df_teamscores.add_prefix("__D__").add_suffix("__D__")
    return df_teamscores

@config_decorators.exit_program(log_filter=lambda args: {k: args[k] for k in ('sr_gameday_output_calculate',)})
def get_correction(sr_snowflake_account: pd.Series, sr_gameday_output_calculate: pd.Series) -> Tuple[str,int]:
    ...
    df_correction = snowflake_execute(sr_snowflake_account,sql.VW_CORRECTION_QUERY,sql.DATABASE,(sr_gameday_output_calculate['SEASON_ID'],sr_gameday_output_calculate['GAMEDAY']))
...
def get_mvp_month_race_figure(sr_snowflake_account: pd.Series, sr_gameday_output_calculate: pd.Series) -> Tuple[str, str, int]:
    ...
    df_month_mvp = snowflake_execute(sr_snowflake_account,sql.VW_USER_SCORES_GAMEDAY_MVPMONTH_QUERY,sql.DATABASE,(sr_gameday_output_calculate['SEASON_ID'],sr_gameday_output_calculate['END_YEARMONTH_LOCAL']))
...
def list_mvp_month_race_gameday(sr_snowflake_account: pd.Series, sr_gameday_output_calculate: pd.Series) -> str:
    ...
    df_month_mvp_gamedays = snowflake_execute(sr_snowflake_account,sql.VW_GAMEDAY_CALCULATED_MVPMONTH_QUERY,sql.DATABASE,(sr_gameday_output_calculate['SEASON_ID'],sr_gameday_output_calculate['END_YEARMONTH_LOCAL']))
...
@config_decorators.exit_program(log_filter=lambda args: {k: args[k] for k in ('sr_gameday_output_calculate',)})
def get_mvp_compet_race_figure(sr_snowflake_account: pd.Series, sr_gameday_output_calculate: pd.Series) -> Tuple[str, int]:
    ...
    df_compet_mvp = snowflake_execute(sr_snowflake_account,sql.VW_USER_SCORES_GAMEDAY_MVPCOMPET_QUERY,sql.DATABASE,(sr_gameday_output_calculate['SEASON_ID'],sr_gameday_output_calculate['COMPETITION_LABEL']))
    ...
    return LIST_USER_COMPETITION, len(df_compet_mvp)
...
def list_mvp_compet_race_gameday(sr_snowflake_account: pd.Series, sr_gameday_output_calculate: pd.Series) -> str:
    ...
    df_month_mvp_gamedays = snowflake_execute(sr_snowflake_account,sql.VW_GAMEDAY_CALCULATED_MVPCOMPET_QUERY,sql.DATABASE,(sr_gameday_output_calculate['SEASON_ID'],sr_gameday_output_calculate['COMPETITION_LABEL']))
    df_month_mvp_gamedays['STRING'] = (df_month_mvp_gamedays['GAMEDAY'] + " (" + df_month_mvp_gamedays['NB_PREDICTION'].astype(str) + " __L__predict__L__)")
...
@config_decorators.exit_program(log_filter=lambda args: {k: args[k] for k in ('sr_gameday_output_calculate',)})
def get_parameters(sr_snowflake_account: pd.Series, sr_gameday_output_calculate: pd.Series) -> dict:
    ...
    df_userscores_global = snowflake_execute(sr_snowflake_account,sql.VW_USER_SCORES_GLOBAL_QUERY,sql.DATABASE,(sr_gameday_output_calculate['SEASON_ID'],))
    param_dict['SCORES_GLOBAL_DF'],param_dict['NB_USER_GLOBAL'] = get_scores_global(df_userscores_global)
    
    # we get the gamedays calculated query 
    df_gameday_calculated = snowflake_execute(sr_snowflake_account,sql.VW_GAMEDAY_CALCULATED_QUERY,sql.DATABASE,(sr_gameday_output_calculate['SEASON_ID'],))
    ...
    if param_dict['IS_FOR_RANK'] == 0:
        ...
        param_dict['RANK_PREDICTCHAMP_DF'] = get_predictchamp_ranking(sr_snowflake_account,sr_gameday_output_calculate)
        
    param_dict['LIST_CORRECTION'],param_dict['NB_CORRECTION'] = get_correction(sr_snowflake_account,sr_gameday_output_calculate)
    ...
    if sr_gameday_output_calculate['DISPLAY_COMPET_MVP_RANKING']  == 1:
        LIST_USER_COMPETITION, NB_USER_COMPETITION = get_mvp_compet_race_figure(sr_snowflake_account,sr_gameday_output_calculate)
...
@config_decorators.exit_program(log_filter=lambda args: {k: args[k] for k in ('country','forum')})
def get_parameters_df_management(param_dict: dict, sr_gameday_output_calculate: pd.Series, country: str, forum: str, translations_dict: dict) -> dict:
    ...
    param_df_dict = {}
    param_df_dict['SCORES_DETAILED_DF_URL_'+country+'_'+forum] = output.manage_df(param_dict['SCORES_DETAILED_DF'], country, forum, "table_score_details", sr_gameday_output_calculate, translations_dict)
    param_df_dict['SCORES_GLOBAL_DF_URL_'+country+'_'+forum] = output.manage_df(param_dict['SCORES_GLOBAL_DF'], country, forum, "table_global_scores", sr_gameday_output_calculate, translations_dict)
    param_df_dict['SCORES_GAMEDAY_DF_URL_'+country+'_'+forum] = output.manage_df(param_dict['SCORES_GAMEDAY_DF'], country, forum, "table_gameday_scores", sr_gameday_output_calculate, translations_dict)
    
    if param_dict['IS_FOR_RANK'] == 0:
        param_df_dict['RANK_PREDICTCHAMP_DF_URL_'+country+'_'+forum] = None
    else:
        param_df_dict['RANK_PREDICTCHAMP_DF_URL_'+country+'_'+forum] = output.manage_df(param_dict['RANK_PREDICTCHAMP_DF'], country, forum, "table_predictchamp_ranking", sr_gameday_output_calculate, translations_dict)
    
    return param_df_dict

@config_decorators.exit_program(log_filter=lambda args: {k: args[k] for k in ('country', 'forum','sr_gameday_output_calculate')})
def create_message(param_dict: dict, template:str, translations_dict: dict, country: str, forum: str, sr_gameday_output_calculate: pd.Series) -> Tuple[str,str]:
    ...
    content = output.format_message(template)

    # we replace parameters...
    # list is (replacement_field, replacement_value)
    replacement_substr = [
        ("#MESSAGE_PREFIX_PROGRAM_STRING#", var.MESSAGE_PREFIX_PROGRAM_STRING),
    ...
    conditional_blocks = [ #begin_tag , end_tag, condition
        ...
        ("#IS_SAME_FOR_PREDICTCHAMP_BEGIN#", "#IS_SAME_FOR_PREDICTCHAMP_END#", param_dict['IS_SAME_FOR_PREDICTCHAMP'] == 1),
        ("#IS_NOT_SAME_FOR_PREDICTCHAMP_BEGIN#", "#IS_NOT_SAME_FOR_PREDICTCHAMP_END#", param_dict['IS_SAME_FOR_PREDICTCHAMP'] == 0),
    ...
    for begin_tag, end_tag, condition in conditional_blocks:
        content = output.replace_conditionally_message(content,begin_tag, end_tag, condition)

    #We then translate the content for the country and the forum
    content = output.translate_string(content,country,forum,translations_dict)

    #We finally create a filename related to this content
    file_name = output.define_filename("forumoutput_calculated", sr_gameday_output_calculate, 'txt', country, forum)
    files_manipulation.create_txt(os.path.join(var.TMPF,file_name),content)
...
def process_output_message_calculated(context_dict: dict, sr_gameday_output_calculate: pd.Series):
    ...
    logging.info("OUTPUT -> GENERATING CALCULATED MESSAGE [START]")
    ...
    df_topics = snowflake_execute(sr_snowflake_account,sql.VW_TOPIC_CALCULATE_QUERY,sql.DATABASE,(sr_gameday_output_calculate['SEASON_ID'],))
    list_countries_forums = (df_topics[['FORUM_COUNTRY', 'FORUM_SOURCE']].drop_duplicates().values.tolist())

    # we get all parameters needed
    param_dict = get_parameters(sr_snowflake_account,sr_gameday_output_calculate)
    param_args= [(param_dict,sr_gameday_output_calculate,country,forum,context_dict['lst_output_gameday_template_translations']) for (country,forum) in list_countries_forums]
    results = multithread_run(get_parameters_df_management, param_args)
    for param_df_dict in results:
        param_dict.update(param_df_dict)
    logging.info("OUTPUT -> PARAM RETRIEVED")
    message_args= [(param_dict,context_dict['str_output_gameday_calculation_template_'+country.lower()]
                    ,context_dict['lst_output_gameday_template_translations'],country,forum,
                    sr_gameday_output_calculate) for (country,forum) in list_countries_forums]
    results = multithread_run(create_message, message_args)
    for content,country,forum in results:
        param_dict['MESSAGE_'+country+'_'+forum] = content
    logging.info("OUTPUT -> MESSAGES CREATED")
    logging.info("OUTPUT -> GENERATING CALCULATED MESSAGE [DONE]")
    return param_dict,df_topics